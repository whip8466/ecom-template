...
function relationItems(relation) {
    ...
    if (Array.isArray(relation))
        return relation;
    if (typeof relation.getItems === 'function')
        return relation.getItems();
    if (typeof relation.toArray === 'function')
...
async function ordersRoutes(fastify) {
    fastify.post('/orders', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        ...
        for (const inputItem of input.items) {
            ...
            if (inputItem.colorName) {
                const colorExists = relationItems(product.availableColors).some((c) => c.colorName === inputItem.colorName);
                if (!colorExists) {
        ...
        const fullOrder = await em.findOne('Order', { id: order.id }, { populate: ['address', 'items.product', 'user'] });
    ...
    fastify.get('/user/orders', { preHandler: [fastify.authenticate] }, async (request) => {
        const orders = await fastify.orm.em.fork().find('Order', { user: request.authUser.userId }, { populate: ['address', 'items.product'], orderBy: { createdAt: 'DESC' } });
        return { data: orders.map(toOrderDto) };
    });
    fastify.get('/user/orders/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const id = Number(request.params.id);
        const order = await fastify.orm.em.fork().findOne('Order', { id, user: request.authUser.userId }, { populate: ['address', 'items.product', 'user'] });
        if (!order)
            return reply.code(404).send({ message: 'Order not found' });
        return { data: toOrderDto(order) };
    });
    fastify.get('/admin/orders', { preHandler: [fastify.requireAnyRole([UserRole.ADMIN, UserRole.MANAGER])] }, async () => {
        const orders = await fastify.orm.em.fork().find('Order', {}, { populate: ['address', 'items.product', 'user'], orderBy: { createdAt: 'DESC' } });
        return { data: orders.map(toOrderDto) };
    });
    fastify.get('/admin/orders/:id', { preHandler: [fastify.requireAnyRole([UserRole.ADMIN, UserRole.MANAGER])] }, async (request, reply) => {
        const id = Number(request.params.id);
        const order = await fastify.orm.em.fork().findOne('Order', { id }, { populate: ['address', 'items.product', 'user'] });
        if (!order)
            return reply.code(404).send({ message: 'Order not found' });
        return { data: toOrderDto(order) };
    });
    fastify.patch('/admin/orders/:id/status', { preHandler: [fastify.requireAnyRole([UserRole.ADMIN, UserRole.MANAGER])] }, async (request, reply) => {
