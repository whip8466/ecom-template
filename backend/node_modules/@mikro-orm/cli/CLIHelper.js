...
class CLIHelper {
    ...
    static async getModuleVersion(name) {
        try {
            ...
            try {
                const path = `${core_1.Utils.resolveModulePath(name)}/package.json`;
                const pkg = await (0, promises_1.readFile)(path, { encoding: 'utf8' });
                return core_1.colors.green(JSON.parse(pkg).version);
            }
            catch {
