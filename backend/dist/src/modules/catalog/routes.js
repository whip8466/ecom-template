...
async function catalogRoutes(fastify) {
    fastify.get('/products', async (request) => {
        ...
        const [products, total] = await em.findAndCount('Product', where, {
    ...
    fastify.get('/products/:slug', async (request, reply) => {
        ...
        const em = fastify.orm.em.fork();
        const product = await em.findOne('Product', { slug }, { populate: ['category', 'images', 'availableColors'] });
        if (!product) {
    ...
    fastify.post('/admin/categories', { preHandler: [fastify.requireAnyRole([UserRole.ADMIN, UserRole.MANAGER])] }, async (request, reply) => {
    ...
    fastify.put('/admin/categories/:id', { preHandler: [fastify.requireAnyRole([UserRole.ADMIN, UserRole.MANAGER])] }, async (request, reply) => {
        const input = categorySchema.parse(request.body);
    ...
    fastify.delete('/admin/categories/:id', { preHandler: [fastify.requireAnyRole([UserRole.ADMIN])] }, async (request, reply) => {
        const id = Number(request.params.id);
        const em = fastify.orm.em.fork();
        const category = await em.findOne('Category', { id });
        if (!category)
    ...
    fastify.post('/admin/products', { preHandler: [fastify.requireAnyRole([UserRole.ADMIN, UserRole.MANAGER])] }, async (request, reply) => {
    ...
    fastify.put('/admin/products/:id', { preHandler: [fastify.requireAnyRole([UserRole.ADMIN, UserRole.MANAGER])] }, async (request, reply) => {
        const input = productSchema.parse(request.body);
        const id = Number(request.params.id);
        const em = fastify.orm.em.fork();
    ...
    fastify.delete('/admin/products/:id', { preHandler: [fastify.requireAnyRole([UserRole.ADMIN])] }, async (request, reply) => {
        const id = Number(request.params.id);
        const em = fastify.orm.em.fork();
